...
def copy_conv(sess, tftensor, layer):
    ...
    W = sess.graph.get_tensor_by_name(
        '{}/conv2d_params:0'.format(tftensor)).eval()
...
def copy_bn(sess, tftensor, layer):
    ...
    gamma = sess.graph.get_tensor_by_name(
        '{}/gamma:0'.format(tftensor)).eval()
    beta = sess.graph.get_tensor_by_name(
        '{}/beta:0'.format(tftensor)).eval()
    avg_mean = sess.graph.get_tensor_by_name(
        '{}/moving_mean:0'.format(tftensor)).eval()
    avg_var = sess.graph.get_tensor_by_name(
        '{}/moving_variance:0'.format(tftensor)).eval()
    eps = sess.graph.get_operation_by_name(tftensor).get_attr(
        'variance_epsilon')
...
def copy_inception(sess, model):
    ...
    for m in ['mixed', 'mixed_1', 'mixed_2']:
        ...
        copy_bn(
            sess, '{}/conv/batchnorm'.format(m),
            getattr(model, m).conv.bn_conv)

        for t in ['tower', 'tower_1', 'tower_2']:
            copy_conv(
                sess, '{}/{}/conv'.format(m, t),
                getattr(getattr(model, m), t).conv)
            copy_bn(
                sess, '{}/{}/conv/batchnorm'.format(m, t),
                getattr(getattr(model, m), t).bn_conv)

            if t == 'tower' or t == 'tower_1':
                copy_conv(
                    sess, '{}/{}/conv_1'.format(m, t),
                    getattr(getattr(model, m), t).conv_1)
                copy_bn(
                    sess, '{}/{}/conv_1/batchnorm'.format(m, t),
                    getattr(getattr(model, m), t).bn_conv_1)

            if t == 'tower_1':
                copy_conv(
                    sess, '{}/{}/conv_2'.format(m, t),
                    getattr(getattr(model, m), t).conv_2)
                copy_bn(
                    sess, '{}/{}/conv_2/batchnorm'.format(m, t),
                    getattr(getattr(model, m), t).bn_conv_2)
    ...
    for m in ['mixed_4', 'mixed_5', 'mixed_6', 'mixed_7']:
        ...
        for t in ['tower', 'tower_1', 'tower_2']:
            ...
            if t == 'tower_1':
                copy_conv(
                    sess, '{}/{}/conv_3'.format(m, t),
                    getattr(getattr(model, m), t).conv_3)
                copy_bn(
                    sess, '{}/{}/conv_3/batchnorm'.format(m, t),
                    getattr(getattr(model, m), t).bn_conv_3)
                copy_conv(
                    sess, '{}/{}/conv_4'.format(m, t),
                    getattr(getattr(model, m), t).conv_4)
                copy_bn(
                    sess, '{}/{}/conv_4/batchnorm'.format(m, t),
                    getattr(getattr(model, m), t).bn_conv_4)
    ...
    for m in ['mixed_9', 'mixed_10']:
        ...
        for t in ['tower', 'tower_1', 'tower_2']:
            ...
            if t == 'tower' or t == 'tower_1':
                copy_conv(
                    sess, '{}/{}/mixed/conv'.format(m, t),
                    getattr(getattr(model, m), t).mixed.conv.conv)
                copy_bn(
                    sess, '{}/{}/mixed/conv/batchnorm'.format(m, t),
                    getattr(getattr(model, m), t).mixed.conv.bn_conv)
                copy_conv(
                    sess, '{}/{}/mixed/conv_1'.format(m, t),
                    getattr(getattr(model, m), t).mixed.conv_1.conv_1)
                copy_bn(
                    sess, '{}/{}/mixed/conv_1/batchnorm'.format(m, t),
                    getattr(getattr(model, m), t).mixed.conv_1.bn_conv_1)
    ...
    w = sess.graph.get_operation_by_name(
        'softmax/logits/MatMul').inputs[1].eval()
...
def set_tf_params(model, downloads_dir, write_graph=False):
    ...
    with tf.gfile.FastGFile(os.path.join(
        ...
        if write_graph:
            # Write graph to file so that it can be visualized using
            # TensorBoard
            tf.summary.FileWriter('data', graph=graph_def)
